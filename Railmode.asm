...
;
	...
	countb		DS	1 ;
	countw		DS	1;
	;reset		DS	1 ;	if the value is %0010, next time it tries to photosense, it returns to "startbutton" subroutine
;
@CODE
...
;__________________________________________________________________________________	
	initialposition: ;set to the initial position
			LOAD	R3  [R5+INPUT]
			CMP 	R3  %01000
			BEQ		returnfrominit  ;if already in the initial position - wait for start button
			LOAD	R3	slow		
...
;__________________________________________________________________________________	
	photosense : ;Thrower - read the sensor
			...
			MOD 	R0	256				;erase the last 8 bits from the other analogue input                                             ;DIV = real PP2 photosensor, MOD = slider on emulator
...
;__________________________________________________________________________________________________________
	pushdisk: ;Thrower design - push slow or fast
			...
			CMP		R3	%0100
			BEQ		returnfrompush			;iff the button went from pressed to unpressed => stop the revolving aka go to begin and wait for buttons ;;;;;;;;;;83t4py793t4p793t4pyh89q3t489[q3t4y89uh89q3t4guq3t45h89[qt5u CHANGE TO PHOTOSENSE/WAITBEFORE1
	...
	off_wait:
			LOAD    R4  [R5+TIMER]      ; wait off for the time that was calculated before
			CMP 	R4	R3
			BMI		returnfrompush				;just return to looping in PWM for the motor
			BRA		off_wait
